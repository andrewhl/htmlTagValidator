...
function peg$parse(input, options) {
    ...
    var peg$FAILED = {},
            ...
            if (ls === null || _u.textNode(ls) === "") {
                ...
                return {
                    error:
                        "The " +
                            esc("DOCTYPE") +
                            " definition for an HTML 5 document should be " +
                            esc.val("html"),
                };
            }
            return {
        ...
        peg$c27 = function(iot, ic, ict) {
            ...
            if ((err = validateSpecialTag(iot, ic, ict)) !== true) {
                ...
                (err = isValidChildren(iot.name, iot.attributes, ic)) !== true
            ) {
                return error(err.error);
            }
            return {
        ...
        peg$c30 = function(sot, sc, sct) {
            ...
            if ((err = validateSpecialTag(sot, sc, sct)) !== true) {
                ...
                (err = isValidChildren(sot.name, sot.attributes, sc)) !== true
            ) {
                return error(err.error);
            }
            return {
        ...
        peg$c34 = function(st) {
            return (
                ["script", "style", "title", "iframe"].indexOf(
                    _u.tagify(st),
                ) !== -1
            );
        },
        ...
        peg$c44 = function(otn, sp, c, ctn) {
            ...
            if (!otn.back) {
                return error(
                    "The " +
                        esc(otn.name) +
                        " element is missing part of its opening tag",
                );
                ...
                return error(
                    "The " +
                        esc(otn.name) +
                        " element is missing part of its closing tag",
                );
            } else if (otn.name !== ctn.name) {
                return error(
                    "Expected open tag " +
                        esc(otn.name) +
                        " to match closing tag " +
                        esc(ctn.name) +
                        "",
                );
            } else if (
                _u.has(
                    (attrs = checkAttributes(otn.name, otn.attributes, c)),
                    "error",
                )
            ) {
            ...
            return error("The " + esc(otn.name) + " tag is a void element and should not have a closing tag");
                ...
                (err = isValidChildren(otn.name, otn.attributes, c)) !== true
            ) {
                return error(err.error);
            }
            return {
        ...
        peg$c45 = function(t, attrs, cl, e) {
            ...
            if (!canBeSelfClosing(t)) {
                return error(
                    "" + esc(t) + "" + " is not a valid self closing tag",
                );
            }

            if (
                _u.has(
                    (attrs = checkAttributes(t, _u.collapse(attrs))),
                    "error",
                )
            ) {
        ...
        peg$c47 = function(ot) {
            ...
            if (!ot.back) {
                ...
                return error(
                    "" + esc(ot.name) + "" + " is not a valid self closing tag",
                );
            }
            ...
            */
                return error(
                    "The XHTML self-closing tag format for " +
                        esc(ot.name) +
                        " is not allowed in HTML 5",
                );
            } else if (
                _u.has(
                    (attrs = checkAttributes(ot.name, ot.attributes)),
                    "error",
                )
            ) {
                return error(attrs.error);
        ...
        peg$c57 = function(tns, tne) {
            ...
            return _u.option("settings/preserveCase", null, codex)
                ? _u.textNode(tn)
                : _u.tagify(tn);
        },
        ...
        peg$c91 = function(com, cc) {
            if (cc === null) {
                return error(
                    "Found an open HTML comment tag without a closing tag",
                );
            }
            return com;
        },
        ...
        peg$c98 = function(cb) {
            ...
            if (tn.indexOf("--") !== -1) {
                return error(
                    "Cannot have two or more consecutive hyphens " +
                        esc.val("--") +
        ...
        peg$c101 = function(cons, com, cone) {
            ...
            if (cone === null) {
                return error(
                    "Conditional comment start tag found without conditional comment end tag",
                );
            } else if (cons === null) {
                return error(
                    "Conditional comment end tag found without conditional comment start tag",
                );
            } else {
                condition = cons;
            }
    ...
    function isSelfClosing(tag, path) {
        ...
        return tags != null
            ? _u.customTest.apply(this, [usePath, tags, [tag]])
            : false;
    }
    ...
    function isAttributeAllowed(tag, attribute, value) {
        var i,
            ...
            attrTest = function(tst) {
                return _u.customTest.apply(that, [
                    "attributes/" + tst,
                    props[tst],
                    [attribute, value],
                ]);
    ...
    function checkAttributes(tag, attributes, contents) {
        ...
        if (_u.has(props, "conditional")) {
            ...
            for (i = 0, len = ref.length; i < len; i++) {
                if (
                    _u.isFunc(ref[i]) &&
                    (rule = ref[i].apply(this, [attributes, _u, codex])) !==
                        true
                ) {
                    return rule;
                }
        ...
        switch (tag) {
            case "head":
                ...
                if (countTitle < 1) {
                    return {
                        error:
                            "The document will not validate as HTML if you omit the " +
                                esc("title") +
                                " tag in the document " +
                                esc("head") +
                                " section",
            ...
            default:
                if (_u.isArray(children) && children.length > 0) {
                    countLink = _u.countWhere(children, {
                        type: "element",
                        name: "link",
                    });
                    if (countLink > 0) {
                        return {
                            error:
                                "The " +
                                    esc("link") +
                                    " element goes only in the " +
                                    esc("head") +
                                    " section of an HTML document",
                        };
                    }
                    countMeta = _u.countWhere(children, {
                        type: "element",
                        name: "meta",
                    });
                    if (countMeta > 0) {
                    ...
                    if (
                        _u.find(children, function(child) {
                            if (
                                child["type"] === "style" &&
                                !_u.has(child.attributes, "scoped")
                            ) {
                                return true;
                            }
                            return false;
                        ...
                        return {
                            error:
                                "If the " +
                                    esc.attr("scoped") +
                                    " attribute is not used, each " +
                                    esc("style") +
                                    " tag must be located in the " +
                                    esc("head") +
                                    " section",
                        };
                    }
